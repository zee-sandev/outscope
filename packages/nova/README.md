...
```bash
npm install @outscope/nova hono @orpc/contract @orpc/server zod reflect-metadata
```
...
import { Controller, Implement } from '@outscope/nova'
...
const openApiDoc = await generator.generate(contract, {
  info: {
    title: 'My API',
    version: '1.0.0',
    description: 'API built with @outscope/nova',
  },
  servers: [
    {
      url: '/api',
